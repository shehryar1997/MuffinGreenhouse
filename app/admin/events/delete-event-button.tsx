...
export function DeleteEventButton({ title, action }: { title: string; action: () => Promise<EventActionResult> }) {
  ...
  function onClick() {
    if (isPending) return
    if (!confirm(`Delete "${title}"?\n\nThis can't be undone. (Events that already have bookings can't be deleted; cancel them instead.)`)) return
    setError(null)
    startTransition(async () => {
      try {
        ...
        setError("Couldn't delete the event. Check your connection and try again.")
      }
    })
  }
